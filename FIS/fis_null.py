...
class fis_tree_null():
    def __init__(self, fitted_clf,train_x,train_y, protected_attribute, protected_value):
        ...
        self.eqop_at_node = {}
        self.dp_at_node = {}
        self.eqop_at_node_null = {}
        self.dp_at_node_null = {}
    ...
    def calculate_fairness_at_each_node(self):
        ...
        for n in range(self.n_nodes):
            if self.children_left[n] != self.children_right[n]:
                ...
                self.eqop_at_node_null[n] = 1 - fairness_rndm(X_left, y_left, X_right, y_right,self.number_of_features,0,1)
                self.dp_at_node_null[n] = 1 - fairness_rndm(X_left, y_left, X_right, y_right,self.number_of_features,0,1)
                self.eqop_at_node[n] = self.eqop_at_node[n] = fairness(X_left, y_left, X_right, y_right,self.number_of_features,0,1)
                self.dp_at_node[n] = self.dp_at_node[n] = fairness(X_left, y_left, X_right, y_right,self.number_of_features,0,2)
    ...
    def calculate_fairness_importance_score(self):
        for i in range(self.n_nodes):
            ...
            if self.children_right[i] != self.children_left[i]:
                self._fairness_importance_score_dp[self.feature[i]] += ((self.dp_at_node[i] - self.dp_at_node_null[i])*len(self.samples_at_node[i])/len(self.samples_at_node[0]))
                self._fairness_importance_score_eqop[self.feature[i]] \
                    += ((self.eqop_at_node[i] - self.eqop_at_node_null[i])\
