...
for b in signals:
    ...
    for i in range (iterations):
        ...
        for k in range(total_features-1):
            result_df = result_df.append({'fis_dp':fis_dp[k],'fis_eqop':fis_eqop[k],'dp_root':fis_root_dp[k],'eq_root':fis_root_eqop[k],'fis_root_dp':f_forest.dp_at_node[1],'fis_root_eqop':f_forest.eqop_at_node[1],'feature_root':f_forest.fitted_clf.tree_.feature[0],'stn':stn}, ignore_index=True)
...
total_features = elements_per_group * 4 + 1
for b in signals:
    result_df = pd.DataFrame(columns=['dp','eqop','depth','stn'])
    x, z, y, beta, stn = toy_4group(elements_per_group,total_samples,0.5,b)
    for d in depth:
        for i in range(iterations):
        ...
        #parameters = {'max_features':[0.5, 0.6, 0.7, 0.8]}
            ...
            eqop = 1 - util.eqop(x_with_protected,y,clf.predict(x),total_features-1,0)
            dp = 1 - util.DP(x_with_protected,y,clf.predict(x),total_features-1,0)
            result_df = result_df.append({'dp':dp, 'eqop':eqop,'depth':d,'stn':stn}, ignore_index=True)
    name = "depth"+"dp_eq"+str(total_samples)+"_"+str(stn)+".csv"
    result_df.to_csv(name)
